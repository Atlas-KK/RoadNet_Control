...
详情页签为核实详情、关联告警、核实记录和关联处置。核实详情在宽屏采用左右布局：左侧展示事件媒体、位置、摄像机、AI置信度、关联告警和精简系统信息，右侧展示核实任务、核实结果与人工订正表单；视口宽度小于1180px时自动切换为上下布局。不得再单独展示与媒体卡片、页头重复的整块“事件信息”。核实结果包括确认事件、判定误报、信息不足持续观察。

AI建议等级仅在页头明确标识，人工确认等级仅在核实表单或已完成的核实结果中展示，两者不得混用。人工订正字段按“核实结论、位置订正、影响范围、补充说明”分组，允许修正事件类型、位置、方向、车道、车辆、伤亡、危化品、等级和备注。历史结论只追加订正，不覆盖。

视频失败时保留事件卡片、关键帧降级标识和受控证据引用，不展示算法信息和文字证据；证据过期显示归档状态。证据只保存evidenceId和受控引用，车牌默认脱敏、人脸默认模糊。
...
16. Given视频失败，When打开详情，Then事件卡片、关键帧降级标识和受控证据引用仍可支持核实，且不展示算法信息和文字证据。
