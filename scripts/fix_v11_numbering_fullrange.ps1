...
function Find-Paragraph([object]$document, [string]$text) {
...
try {
  ...
  foreach ($pair in $groups) {
    $first = Find-Paragraph $doc $pair[0]
    $last = Find-Paragraph $doc $pair[1]
...
'Full paragraph numbering groups reset successfully.'
